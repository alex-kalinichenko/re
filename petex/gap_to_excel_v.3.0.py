...
def get_date_list(n_dates):
    #date_list = []
    date_list = DoGet(petex, "GAP.MOD[i].SEP[j].PREDRES.DATES[$].DATESTR").split('|')
...
##    OSOpenFile(petex, cwd + file_name, 'PROSPER')
    ...
    #OSOpenFile(petex, r'D:\work\0301_3.8\PES_ACH.gap', 'gap')
    ...
    param_list_well = ['CUMOIL','CUMWAT','CUMGAS','OILRATE', 'GASRATE','WATRATE','LIQRATE','MANPRES','MANTEMP',\
                       'FWHP','FBHP','DrawDown','WCT','MixtureVelocity','ErosionalVelocity','AVGGASRATE'] #'OPFREQ'
    # для Joint
    param_list_joint = ['OILRATE','GASRATE','WATRATE','LIQRATE','PRES','WCT','TEMP']
...
## ====================  КОНЕЦ СЕКЦИИ ПАРАМЕТРОВ НАСТРОЙКИ СКРИПТА ======================
    ...
    n_dates = int((DoGet(petex, "GAP.MOD[i].SEP[j].PREDRES.DATES.COUNT")[:-1]))
    print(n_dates)
